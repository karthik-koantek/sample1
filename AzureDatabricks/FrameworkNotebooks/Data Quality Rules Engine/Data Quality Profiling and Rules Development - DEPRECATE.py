...
if snb.profile == True:
  try:
    expectation_suite, validation_results = profileGreatExpectations(dfGE, snb.tableName)
    expectation_list = formatGreatExpectationsList(expectation_suite)
  except Exception as e:
    err = {
...
failures = [x for x in validation_results.results if not x.success]
if len(failures) > 0:
  print(failures)
...
expectation_suite = dfGE.get_expectation_suite(discard_failed_expectations=True)
expectation_list = formatGreatExpectationsList(expectation_suite)
expectation_list
...
try:
  saveExpectationList(snb.tableName, expectation_list)
except Exception as e:
  err = {
