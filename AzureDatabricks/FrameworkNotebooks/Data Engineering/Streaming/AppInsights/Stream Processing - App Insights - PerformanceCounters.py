...
# MAGIC %md # Stream Processing - App Insights - PerformanceCounters
# MAGIC
...
appInsightsDataType = "PerformanceCounters"
...
try:
  ...
  err = {
    "sourceName": "Stream Processing - App Insights - PerformanceCounters: Read Stream",
    "errorCode": "100",
...
performanceCountersDF, performanceCountersPerformanceCounterFlattenedDF = flatten_AppInsights_df_PerformanceCounters (df)
...
rawDF = (df \
  .writeStream \
  .queryName("performancecounters raw") \
  .trigger(once=True) \
  .format("json") \
  .option("checkpointLocation", rawCheckpointPath + "/performancecountersRaw/" + dateToProcess) \
  .outputMode("append") \
  .start(rawDataPath + "/performancecounters/" + dateToProcess)
)
...
performanceCountersDFQueryZone = (performanceCountersDF \
  .writeStream \
  .queryName("performancecounters query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/performancecountersQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/performancecounters")
)

performanceCountersPerformanceCounterFlattenedDFDFQueryZone = (performanceCountersPerformanceCounterFlattenedDF \
  .writeStream \
  .queryName("performancecounters performancecounters query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/performancecountersperformancecountersQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/performancecountersperformancecounters")
)
