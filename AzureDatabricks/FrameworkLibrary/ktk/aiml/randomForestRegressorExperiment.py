...
class RandomForestRegressorExperiment(RegressionExperiment):
    def __init__(self, snb, experimentName=""):
        super().__init__(snb, experimentName)
        if experimentName == "":
            self.experimentName = "Train Random Forest Regression for Table {0}".format(snb.tableName.replace(".","_"))
        else:
    ...
    def trainingRun(self, df, runName=""):
        ...
        with mlflow.start_run(run_name=self.runName) as run:
            ...
            startTime = time.time()
            rf = RandomForestRegressor(featuresCol="features", labelCol=self.label)
            print("Step 4: Model: {0}".format(rf))
            ...
            paramGrid = ParamGridBuilder().addGrid(rf.regParam, paramGridList).build()
            ...
            crossval = CrossValidator(estimator=pipeline,estimatorParamMaps=paramGrid,evaluator=RegressionEvaluator(labelCol=label,predictionCol=predictionColumn),numFolds=folds)
            ...
            mlflow.spark.log_model(spark_model=model.bestModel, artifact_path="model-file", registered_model_name=experimentName)
