...
class XGBoostRegressorExperiment(RegressionExperiment):
    def __init__(self, snb, experimentName=""):
        super().__init__(snb, experimentName)
        if experimentName == "":
            self.experimentName = "Train XGBoost Regression for Table {0}".format(snb.tableName.replace(".","_"))
        else:
    ...
    def trainingRun(self, df, runName=""):
        ...
        with mlflow.start_run(run_name=self.runName) as run:
            ...
            startTime = time.time()
            xgb = XgboostRegressor(featuresCol="features", labelCol=self.label)
            print("Step 4: Model: {0}".format(xgb))
            ...
            paramGrid = ParamGridBuilder().addGrid(xgb.regParam, paramGridList).build()
            ...
            crossval = CrossValidator(estimator=pipeline,estimatorParamMaps=paramGrid,evaluator=RegressionEvaluator(labelCol=self.label,predictionCol=self.predictionColumn),numFolds=self.folds)
