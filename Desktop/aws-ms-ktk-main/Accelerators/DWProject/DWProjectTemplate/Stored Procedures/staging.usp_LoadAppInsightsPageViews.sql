CREATE PROCEDURE staging.usp_LoadAppInsightsPageViews
AS
BEGIN
-- ====================================================================
-- Author:		Eddie Edgeworth
-- Create Date: 08-22-19
-- Description: Loads the contents of staging.vAppInsightsPageViews into dbo.AppInsightsPageViews.
-- ====================================================================
	...
	BEGIN TRY
		...
		MERGE dbo.AppInsightsPageViews AS tgt
		USING
		(
			SELECT DISTINCT
			...
			,COALESCE(CONVERT(VARCHAR(200),ViewName),'') AS ViewName
			,COALESCE(CONVERT(VARCHAR(1000),ViewUrl),'') AS ViewUrl
			,COALESCE(CONVERT(VARCHAR(50),ViewUrlDataHost),'') AS ViewUrlDataHost
			,COALESCE(Count,0) AS [Count]
			FROM staging.vAppInsightsPageViews
		) AS src ON tgt.Tool=src.Tool
				AND tgt.ContextSessionId=src.ContextSessionId
				AND tgt.[Date]=src.[Date]
				AND tgt.ViewName=src.ViewName
				AND tgt.ViewUrl=src.ViewUrl
	...
	BEGIN CATCH
		...
		RAISERROR('[staging].[usp_LoadAppInsightsPageViews] failed with error %d at line %d: %s',16,1,@Error,@ErrorLine,@Message) WITH NOWAIT;
