...
dbutils.widgets.text(name="outputTableName", defaultValue="tweetswithsentiment", label="Scored Table Name")
...
scoredTableName = dbutils.widgets.get("outputTableName")
...
if destination == "silverprotected":
  basePath = silverProtectedBasePath
else:
  basePath = silverGeneralBasePath
checkpointPath = "{0}/checkpoint/{1}/{2}".format(basePath, externalSystem, scoredTableName)
sentimentDataPath = "{0}/{1}/{2}".format(basePath, externalSystem, scoredTableName)
...
# MAGIC val scoredTableName = dbutils.widgets.get("outputTableName")
...
# MAGIC %python
# MAGIC refreshed = refreshTable(tableName)
# MAGIC if refreshed == False:
# MAGIC   log_notebook_end(notebookLogGuid, 0, server, database, login, pwd)
# MAGIC   dbutils.notebook.exit("Table does not exist")
