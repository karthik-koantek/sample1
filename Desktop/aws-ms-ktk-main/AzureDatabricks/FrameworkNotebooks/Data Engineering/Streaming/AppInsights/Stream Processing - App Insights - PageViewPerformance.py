...
# MAGIC %md # Stream Processing - App Insights - PageViewPerformance
# MAGIC
...
appInsightsDataType = "PageViewPerformance"
...
try:
  ...
  err = {
    "sourceName": "Stream Processing - App Insights - PageViewPerformance: Read Stream",
    "errorCode": "100",
...
pageViewPerformanceDF, pageViewPerformanceClientPerformanceFlattenedDF = flatten_AppInsights_df_PageViewPerformance (df)
...
rawDF = (df \
  .writeStream \
  .queryName("pageviewperformance raw") \
  .trigger(once=True) \
  .format("json") \
  .option("checkpointLocation", rawCheckpointPath + "/pageviewperformanceRaw/" + dateToProcess) \
  .outputMode("append") \
  .start(rawDataPath + "/pageviewperformance/" + dateToProcess)
)
...
pageViewPerformanceDFQueryZone = (pageViewPerformanceDF \
  .writeStream \
  .queryName("pageviewperformance query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/pageviewperformanceQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/pageviewperformance")
)

pageViewPerformanceClientPerformanceDFQueryZone = (pageViewPerformanceClientPerformanceFlattenedDF \
  .writeStream \
  .queryName("pageviewperformance clientperformance query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/pageviewperformanceclientperformanceQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/pageviewperformanceclientperformance")
)
