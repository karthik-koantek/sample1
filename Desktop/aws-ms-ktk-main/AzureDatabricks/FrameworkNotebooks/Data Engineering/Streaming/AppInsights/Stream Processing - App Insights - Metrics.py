...
# MAGIC %md # Stream Processing - App Insights - Metrics
# MAGIC
...
try:
  ...
  err = {
    "sourceName": "Stream Processing - App Insights - Metrics: Read Stream",
    "errorCode": "100",
...
metricsDF, metrics_metricDF, metrics_contextCustomDimensionsDF,metrics_contextCustomMetricsDF = flatten_AppInsights_df_Metrics (df)
...
rawDF = (df \
  .writeStream \
  .queryName("metrics raw") \
  .trigger(once=True) \
  .format("json") \
  .option("checkpointLocation", rawCheckpointPath + "/metricsRaw/" + dateToProcess) \
  .outputMode("append") \
  .start(rawDataPath + "/metrics/" + dateToProcess)
)
...
metricsDFQueryZone = (metricsDF \
  .writeStream \
  .queryName("metrics query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/metricsQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/metrics")
)

metricsMetricsDFQueryZone = (metrics_metricDF \
  .writeStream \
  .queryName("metrics metrics query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/metricsMetricsQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/metricsMetrics")
)

metricsCustomDimensionsDFQueryZone = (metrics_contextCustomDimensionsDF \
  .writeStream \
  .queryName("metrics custom dimensions query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/metricsCustomDimensionsQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/metricsCustomDimensions")
)

metricsCustomMetricsDFQueryZone = (metrics_contextCustomMetricsDF \
  .writeStream \
  .queryName("metrics custom metrics query") \
  .trigger(once=True) \
  .format("delta") \
  .option("checkpointLocation", queryCheckpointPath + "/metricsCustomMetricsQuery") \
  .outputMode("append") \
  .start(queryDataPath + "/metricsCustomMetrics")
)
