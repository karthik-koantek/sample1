...
class Notebook():
  def __init__(self, cloud="AWS", logMethod=2):
    self.setContext()
    #self.setFrameworkDB("internal")
    self.setDataLakeStorage(secretScope="internal", cloud=cloud) #cloud = "AWS" #AWS, GCP
    self.LogMethod = logMethod # 0 = None, 1 = Azure SQL, 2 = DB, 3 = Both
  ...
  def setDataLakeStorage(self, secretScope, cloud): 
    dbutils = self.get_dbutils()
    scope = [s for s in dbutils.secrets.listScopes() if s.name == "internal"]
    if scope == []:
      err = "Secret Scope '{0}' needs to be created for Ascend storage mappings".format(secretScope)
      raise Exception(err)
    ...
    if cloud == "Azure":
      ...
      #implement AWS S3 storage here
      #Bronze Zone
      self.setFromSecret(secretScope, "AWSBronzeBucketName", "AWSBronzeBucketName")
      #Silver Zone
      self.setFromSecret(secretScope, "AWSSilverGeneralBucketName", "AWSSilverGeneralBucketName")
      self.setFromSecret(secretScope, "AWSSilverProtectedBucketName", "AWSSilverProtectedBucketName")
      #Gold Zone
      self.setFromSecret(secretScope, "AWSGoldGeneralBucketName", "AWSGoldGeneralBucketName")
      self.setFromSecret(secretScope, "AWSGoldProtectedBucketName", "AWSGoldProtectedBucketName")
      
      self.create_top_level_paths()
      self.BronzeBasePath = "s3a://{0}/raw/".format(self.AWSBronzeBucketName)
      self.SilverGeneralBasePath = "s3a://{0}/silvergeneral".format(self.AWSSilverGeneralBucketName)
      self.SilverProtectedBasePath = "s3a://{0}/silverprotected".format(self.AWSSilverProtectedBucketName)
      self.ArchiveBasePath = "s3a://{0}/archive".format(self.AWSSilverProtectedBucketName)
      self.GoldGeneralBasePath = "s3a://{0}/goldgeneral".format(self.AWSGoldGeneralBucketName)
      self.GoldProtectedBasePath = "s3a://{0}/goldprotected".format(self.AWSGoldProtectedBucketName)
      self.SandboxBasePath = "s3a://{0}/defaultsandbox".format(self.AWSGoldGeneralBucketName)
      
    # elif self.Cloud == "GCP":
          
        #implement GCP storage here
    # else:
        # 
        
  def create_top_level_paths(self):
    dbutils = self.get_dbutils()
    dbutils.fs.mkdirs(f"s3a://{self.AWSBronzeBucketName}" + '/raw')
    dbutils.fs.mkdirs(f"s3a://{self.AWSSilverGeneralBucketName}" + '/silvergeneral')
    dbutils.fs.mkdirs(f"s3a://{self.AWSSilverProtectedBucketName}" + '/silverprotected')
    dbutils.fs.mkdirs(f"s3a://{self.AWSSilverGeneralBucketName}" + '/archive')
    dbutils.fs.mkdirs(f"s3a://{self.AWSGoldGeneralBucketName}" + '/goldgeneral')
    dbutils.fs.mkdirs(f"s3a://{self.AWSGoldProtectedBucketName}" + '/goldprotected')
    dbutils.fs.mkdirs(f"s3a://{self.AWSGoldGeneralBucketName}" + '/defaultsandbox')
