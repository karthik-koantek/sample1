...
#region begin Notebooks
    ...
    if ($NotebookDeploymentMode -eq "repos" -or $NotebookDeploymentMode -eq "both") {
        ...
        try {
            New-DatabricksRepo -BaseURI $URIBase -Token $RepoToken -URL $RepoURL -Provider $RepoProvider -Path "/Repos/Production/Koantek"
        } catch {"Repo creation timed out, sleeping for 60 seconds to allow for git operation to complete."} finally {Start-Sleep(60)}
        try {
            New-DatabricksRepo -BaseURI $URIBase -Token $RepoToken -URL $RepoURL -Provider $RepoProvider -Path "/Repos/Development/Koantek"
        } catch {"Repo creation timed out, sleeping for 60 seconds to allow for git operation to complete."} finally {Start-Sleep(60)}
        
        $ProductionRepo = ((Get-DatabricksRepos -BaseURI $URIBase -Token $Token -PathPrefix "/Repos/Production/Koantek").Content | ConvertFrom-Json).repos
        $DevRepo = ((Get-DatabricksRepos -BaseURI $URIBase -Token $Token -PathPrefix "/Repos/Development/Koantek").Content | ConvertFrom-Json).repos
        ...
        try {
            Update-DatabricksRepo -BaseURI $URIBase -Token $RepoToken -RepoId $ProductionRepo.Id -Branch "master"
        } catch {"Repo update timed out, sleeping for 60 seconds to allow for git operation to complete."} finally {Start-Sleep(60)}
    }    
