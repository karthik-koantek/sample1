CREATE PROCEDURE [dbo].[HydrateAppInsights]
 @ProjectName VARCHAR(255)
,@SystemName VARCHAR(255)
,@SystemSecretScope VARCHAR(255) = 'external'
,@SystemOrder INT = 10
...
,@JobOrder INT = 10
,@JobIsActive BIT = 1
,@AppInsightsInstanceName VARCHAR(255)
,@StreamingNotebookPath VARCHAR(255) = '../Data Engineering/Streaming/AppInsights/Stream Processing - App Insights - Event'
AS
BEGIN
	...
	IF @AppInsightsInstanceName IS NULL OR @AppInsightsInstanceName = ''
	BEGIN
		SET @ErrorMessage = 'The value for the parameter @AppInsightsInstanceName is not supported.' + CHAR(13) + CHAR(10) + ' ';
		RAISERROR(@ErrorMessage,16,1) WITH NOWAIT;
		SET @Error = @@ERROR;
	END

	IF @StreamingNotebookPath IS NULL OR @StreamingNotebookPath = ''
	BEGIN
		SET @ErrorMessage = 'The value for the parameter @StreamingNotebookPath is not supported.' + CHAR(13) + CHAR(10) + ' ';
		RAISERROR(@ErrorMessage,16,1) WITH NOWAIT;
		SET @Error = @@ERROR;
	END
	...
	SELECT 'appInsightsInstanceName' AS ParameterName, @AppInsightsInstanceName AS ParameterValue
	UNION
	...
	,@StepName=@StreamingNotebookPath
	,@StepOrder=10
	,@StepIsActive=1
